...
| `chinese-commit` 中文 commit 规范 | 英文 type + 中文主题、祈使句、一次一件事、scope 用真实模块名 |
| `domestic-stack` 国内技术栈适配 | uniapp/小程序禁用 DOM API、条件编译跨端、微信登录走官方流程、SpringBoot 分层清晰 |
| `requirement-delivery` 接需求提效 | 先列验收标准再动手、拆 MVP 优先核心路径、复用优先、边做边验证、交付物可直接运行 |
| `large-repo-refactor` 大仓库重构 | 先补测试护栏、行为不变原则、单一变换每步可回退、自动化重构工具优先、改前做影响面分析 |
