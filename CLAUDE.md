...
- 单一变换每步可回退：每次提交只做一种操作（改名/抽函数/移文件），绝不在同一提交里混合多种变换，保证可精确二分定位。
- 自动化重构工具优先：重命名用 IDE "Rename Symbol"，大规模迁移用 codemod；确需脚本替换时先在小范围人工核对再扩大范围。
- 改前做影响面分析：用 Grep 找出所有调用点和跨模块引用；影响超过 3 个模块或涉及对外接口时，先同步团队拿到共识再动手。
